...
func englishOther(year uint32, events *list.List) {
	...
	events.PushBack(Event{t, "Septuagesima Sunday", 1}) // "70th", 3rd Sunday before Lent (9th before Easter)
	t = easter.AddDate(0, 0, -56)
	events.PushBack(Event{t, "Sexagesima Sunday", 1}) // "60th", 2nd Sunday before Lent (8th before Easter)
	t = easter.AddDate(0, 0, -49)
	events.PushBack(Event{t, "Quinquagesima/Shrove/Pork Sunday", 1}) // "50th", last Sunday before Lent (7th before Easter)
	t = easter.AddDate(0, 0, -47)
	events.PushBack(Event{t, "Shrove/Pancake Tuesday", 1}) // day before Lent
	t = easter.AddDate(0, 0, -46)
	events.PushBack(Event{t, "Carnival/Ash Wednesday", 1}) // Lent begins
	t = easter.AddDate(0, 0, -42)
	events.PushBack(Event{t, "Invocabit Sunday", 1}) // "40th", 1st Sunday after Lent (6th before Easter)
	t = easter.AddDate(0, 0, -21)
	events.PushBack(Event{t, "Laetare Sunday", 1}) // 4th before Easter
	t = easter.AddDate(0, 0, -7)
	events.PushBack(Event{t, "Palm Sunday", 1})
	t = easter.AddDate(0, 0, -4)
	events.PushBack(Event{t, "Holy Wednesday", 1})
	t = easter.AddDate(0, 0, -3)
	events.PushBack(Event{t, "Maundy Thursday", 1})
	t = easter.AddDate(0, 0, -2)
	events.PushBack(Event{t, "Good Friday", 1})
	t = easter.AddDate(0, 0, -1)
	events.PushBack(Event{t, "Holy Saturday", 1})
	events.PushBack(Event{easter, "Easter Sunday", 1})
	t = easter.AddDate(0, 0, 1)
	events.PushBack(Event{t, "Easter Monday", 1})
	t = easter.AddDate(0, 0, 6)
	events.PushBack(Event{t, "Easter Saturday", 1})
	t = easter.AddDate(0, 0, 39)
	events.PushBack(Event{t, "Ascension Day", 1})
	t = easter.AddDate(0, 0, 49)
	events.PushBack(Event{t, "Whit/Pentecost Sunday", 1})
	t = easter.AddDate(0, 0, 50)
	events.PushBack(Event{t, "Whit/Pentecost Monday", 1})
	t = easter.AddDate(0, 0, 56)
	events.PushBack(Event{t, "Trinity Sunday", 1})
	t = easter.AddDate(0, 0, 60)
	events.PushBack(Event{t, "Corpus Christi", 1})
	...
	events.PushBack(Event{pascha, "Passover", 1})
	//t = pascha.AddDate(0, 0, 8)
	//events.PushBack(Event{t, "Orthodox Easter Sunday", 1})
	...
	events.PushBack(Event{t, "Mother's Day", 1})
	...
	events.PushBack(Event{t, "Father's Day", 1})
	...
	for i := 0; i < 53; i++ {
		if 13 == t.Day() {
			events.PushBack(Event{t, "Friday the 13th", 1})
		}
		t = t.AddDate(0, 0, 7)
	}
	...
	events.PushBack(Event{t, "National Cat Day (BR, IT)", 1})
	t = find_date(fmt.Sprintf("%04d-%02d-22", year, time.February))
	events.PushBack(Event{t, "National Cat Day (JP)", 1})
	t = find_date(fmt.Sprintf("%04d-%02d-01", year, time.March))
	events.PushBack(Event{t, "National Cat Day (RU)", 1})
	t = find_date(fmt.Sprintf("%04d-%02d-08", year, time.August))
	events.PushBack(Event{t, "International Cat Day", 1})
	events.PushBack(Event{t, "National Cat Day (CA)", 1})
	t = find_date(fmt.Sprintf("%04d-%02d-29", year, time.October))
	events.PushBack(Event{t, "National Cat Day (US)", 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Raspberry Pi", ordinal(int(year-2012), "en")), 1})
	...
	events.PushBack(Event{t, "Pi Day 3.14", 1})
	t = find_date(fmt.Sprintf("%04d-06-28", year))
	events.PushBack(Event{t, "Tau Day 6.28", 1})
	t = find_date(fmt.Sprintf("%04d-07-22", year))
	events.PushBack(Event{t, "Pi Approximation Day 22/7", 1})
	if is_leap(year) {
		t = find_date(fmt.Sprintf("%04d-%02d-09", year, time.November))
	} else {
		t = find_date(fmt.Sprintf("%04d-%02d-10", year, time.November))
	}
	events.PushBack(Event{t, "Pi Approximation Day 314th day", 1})
	...
	events.PushBack(Event{t, "Day of the Programmer 256th day", 1})
	...
	t = t.AddDate(0, 0, -5)
	events.PushBack(Event{t, "Global Payroll Week Begins", 1})
	t = find_nearby_date(fmt.Sprintf("%04d-%02d-%02d", year, time.September, Month3rd), uint32(time.Tuesday))
	events.PushBack(Event{t, "National Day of Recognition for Payroll Professionals", 1})
	...
	events.PushBack(Event{t, "Software Freedom Day", 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Unix", ordinal(int(year-1970), "en")), 1})

	// June 1st, 1974
	// https://en.wikipedia.org/wiki/Diff
	// https://fr.wikipedia.org/wiki/Diff
	t = find_date(fmt.Sprintf("%04d-%02d-01", year, time.June))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Diff", ordinal(int(year-1974), "en")), 1})

	// February 20th, 1991
	// https://en.wikipedia.org/wiki/Python_(programming_language)
	// https://fr.wikipedia.org/wiki/Python_(langage)
	t = find_date(fmt.Sprintf("%04d-%02d-20", year, time.February))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Python", ordinal(int(year-1991), "en")), 1})

	// September 17th, 1991
	// https://en.wikipedia.org/wiki/Linux
	// https://fr.wikipedia.org/wiki/Linux
	t = find_date(fmt.Sprintf("%04d-%02d-17", year, time.September))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Linux", ordinal(int(year-1991), "en")), 1})

	// April 19th, 1993
	// https://en.wikipedia.org/wiki/NetBSD
	// https://fr.wikipedia.org/wiki/NetBSD
	t = find_date(fmt.Sprintf("%04d-%02d-19", year, time.April))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of NetBSD", ordinal(int(year-1993), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Slackware", ordinal(int(year-1993), "en")), 1})

	// September 15th, 1993
	// https://en.wikipedia.org/wiki/Debian
	// https://fr.wikipedia.org/wiki/Debian
	t = find_date(fmt.Sprintf("%04d-%02d-15", year, time.September))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Debian", ordinal(int(year-1993), "en")), 1})

	// November 1st, 1993
	// https://en.wikipedia.org/wiki/FreeBSD
	// https://fr.wikipedia.org/wiki/FreeBSD
	t = find_date(fmt.Sprintf("%04d-%02d-01", year, time.November))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of FreeBSD", ordinal(int(year-1993), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of OpenBSD", ordinal(int(year-1995), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of GIMP", ordinal(int(year-1998), "en")), 1})

	// March 11th, 2002
	// https://en.wikipedia.org/wiki/Arch_Linux
	// https://fr.wikipedia.org/wiki/Arch_Linux
	t = find_date(fmt.Sprintf("%04d-%02d-11", year, time.March))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Arch Linux", ordinal(int(year-2002), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Inkscape", ordinal(int(year-2003), "en")), 1})

	// January 1st, 2004
	// https://en.wikipedia.org/wiki/OpenWrt
	// https://fr.wikipedia.org/wiki/OpenWrt
	t = find_date(fmt.Sprintf("%04d-%02d-01", year, time.January))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of OpenWRT", ordinal(int(year-2004), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Ubuntu", ordinal(int(year-2004), "en")), 1})

	// April 7, 2005
	// https://en.wikipedia.org/wiki/Git
	// https://fr.wikipedia.org/wiki/Git
	t = find_date(fmt.Sprintf("%04d-%02d-07", year, time.April))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Git", ordinal(int(year-2005), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Krita", ordinal(int(year-2005), "en")), 1})

	// August 22, 2005
	// https://en.wikipedia.org/wiki/Alpine_Linux
	// https://fr.wikipedia.org/wiki/Alpine_Linux
	t = find_date(fmt.Sprintf("%04d-%02d-22", year, time.August))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Alpine Linux", ordinal(int(year-2005), "en")), 1})

	// November 10th, 2009
	// https://en.wikipedia.org/wiki/Go_(programming_language)
	// https://fr.wikipedia.org/wiki/Go_(langage)
	t = find_date(fmt.Sprintf("%04d-%02d-10", year, time.November))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of Golang", ordinal(int(year-2009), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of OpenSCAD", ordinal(int(year-2010), "en")), 1})

	// January 25th, 2011
	// https://en.wikipedia.org/wiki/LibreOffice
	// https://fr.wikipedia.org/wiki/LibreOffice
	t = find_date(fmt.Sprintf("%04d-%02d-25", year, time.January))
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of LibreOffice", ordinal(int(year-2011), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Birthday of LibreCAD", ordinal(int(year-2011), "en")), 1})
	...
	events.PushBack(Event{t, "International Talk Like a Pirate Day", 1})
	...
	events.PushBack(Event{t, "Left-Handers' Day", 1})
	...
	events.PushBack(Event{t, "INTERNATIONAL CAPS LOCK DAY", 1})
	...
	events.PushBack(Event{t, "World Radio Day", 1})
	...
	events.PushBack(Event{t, "World Amateur Radio Day", 1})

	// November 27, 1923
	// https://en.wikipedia.org/wiki/History_of_amateur_radio
	t = find_date(fmt.Sprintf("%04d-%02d-27", year, time.November))
	events.PushBack(Event{t, fmt.Sprintf("%s Anniversary of first transatlantic two-way ham contact", ordinal(int(year-1923), "en")), 1})
	...
	events.PushBack(Event{t, "Darwin Day", 1})
	...
	events.PushBack(Event{t, "Nikola Tesla Day", 1})
	...
	events.PushBack(Event{t, "Ada Lovelace Day", 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Anniversary of Sputnik 1 launch", ordinal(int(year-1957), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Anniversary of Apollo 11 lunar landing", ordinal(int(year-1969), "en")), 1})
	t = find_date(fmt.Sprintf("%04d-%02d-11", year, time.December))
	events.PushBack(Event{t, fmt.Sprintf("%s Anniversary of Apollo 17 lunar landing", ordinal(int(year-1972), "en")), 1})
	...
	events.PushBack(Event{t, fmt.Sprintf("%s Anniversary of Hubble launch", ordinal(int(year-1990), "en")), 1})
	...
	if 2027 == year {
		...
		events.PushBack(Event{t, "06:48 1999 AN10 Asteroid Pass", 1})
	}
	if 2028 == year {
		t = find_date("2028-10-26")
		events.PushBack(Event{t, "06:44 1997 XF11 Asteroid Pass", 1})
	}
	...
	if 2036 == year {
		...
		events.PushBack(Event{t, "06:28:16 UTC NTP 32-bit Overflow", 1})
	}
	if 2038 == year {
		t = find_date("2038-01-19")
		events.PushBack(Event{t, "03:14:07 UTC POSIX 32-bit Overflow", 1})
	}
}
