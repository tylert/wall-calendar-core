...
func englishUnitedStates(year uint32) {
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.January, Month3rd), uint32(time.Monday))
	...
	// Fête du 5 mai (US)
	t = find_date(fmt.Sprintf("%d-%02d-05", year, time.May))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.May, Month2nd), uint32(time.Saturday))
	print_date(t, "Armed Forces Week (US)")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.May, Month3rd), uint32(time.Saturday))
	...
	// Jour du drapeau (US)
	t = find_date(fmt.Sprintf("%d-%02d-14", year, time.June))
	...
	t = find_date(fmt.Sprintf("%d-%02d-04", year, time.July))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.November, Month4th), uint32(time.Thursday))
	...
	t = find_date(fmt.Sprintf("%d-%02d-07", year, time.December))
