...
func englishCanada(year uint32) {
	...
	t = find_date(fmt.Sprintf("%d-%02d-01", year, time.January))
	...
	t = find_date(fmt.Sprintf("%d-%02d-17", year, time.January))
	...
	// Conversion de Paul
	t = find_date(fmt.Sprintf("%d-%02d-25", year, time.January))
	...
	// Jour de la marmotte
	t = find_date(fmt.Sprintf("%d-%02d-02", year, time.February))
	...
	// Saint-Valentin
	t = find_date(fmt.Sprintf("%d-%02d-14", year, time.February))
	...
	t = find_date(fmt.Sprintf("%d-%02d-15", year, time.February))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.February, Month3rd), uint32(time.Monday))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-28", year, time.February), uint32(time.Sunday))
	t = t.AddDate(0, 0, -2)
	...
	// Congé de printemps
	// Semaine de relâche
	t = find_date(fmt.Sprintf("%d-%02d-01", year, time.March))
	if t.Weekday() == time.Sunday || t.Weekday() == time.Monday {
		t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.March, Month3rd), uint32(time.Monday))
	} else {
		t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.March, Month2nd), uint32(time.Monday))
	}
	...
	t = find_date(fmt.Sprintf("%d-%02d-17", year, time.March))
	print_date(t, "St. Patrick's Day")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-17", year, time.March), uint32(time.Monday))
	print_date(t, "St. Patrick's Day (CA-NL)")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-23", year, time.April), uint32(time.Monday))
	print_date(t, "St. George's Day (CA-NL)")
	t = find_date(fmt.Sprintf("%d-%02d-23", year, time.April))
	print_date(t, "St. George's Day")
	t = find_date(fmt.Sprintf("%d-%02d-30", year, time.November))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.March, Month2nd), uint32(time.Sunday))
	print_date(t, "02:00 DST Begins (CA, US)")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.November, Month1st), uint32(time.Sunday))
	print_date(t, "02:00 DST Ends (CA, US)")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-31", year, time.March), uint32(time.Sunday))
	print_date(t, "01:00Z DST Begins (EU, GB)")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-31", year, time.October), uint32(time.Sunday))
	print_date(t, "01:00Z DST Ends (EU, GB)")

	// 1st Friday in March
	// https://en.wikipedia.org/wiki/Employee_Appreciation_Day
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.March, Month1st), uint32(time.Friday))
	...
	// Poisson d'avril
	t = find_date(fmt.Sprintf("%d-%02d-01", year, time.April))
	...
	// Journée du Tartan
	t = find_date(fmt.Sprintf("%d-%02d-06", year, time.April))
	print_date(t, "Tartan Day")

	// April 9th
	// https://en.wikipedia.org/wiki/Vimy_Ridge_Day
	// Jour de la crête de Vimy
	t = find_date(fmt.Sprintf("%d-%02d-09", year, time.April))
	...
	// Jour de la Terre
	t = find_date(fmt.Sprintf("%d-%02d-22", year, time.April))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-21", year, time.May), uint32(time.Monday))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.June, Month1st), uint32(time.Sunday))
	...
	t = find_date(fmt.Sprintf("%d-%02d-19", year, time.June))
	...
	t = find_date(fmt.Sprintf("%d-%02d-21", year, time.June))
	...
	// observé
	t = find_date(fmt.Sprintf("%d-%02d-24", year, time.June))
	...
	t = find_date(fmt.Sprintf("%d-%02d-27", year, time.June))
	...
	t = find_date(fmt.Sprintf("%d-%02d-01", year, time.July))
	...
	t = find_date(fmt.Sprintf("%d-%02d-09", year, time.July))
	...
	t = find_date(fmt.Sprintf("%d-%02d-12", year, time.July))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-31", year, time.July), uint32(time.Sunday))
	t = t.AddDate(0, 0, -7)
	...
	t = find_date(fmt.Sprintf("%d-%02d-09", year, time.August))
	print_date(t, "International Day of the World's Indigenous Peoples")

	// 3rd Monday in August
	// https://en.wikipedia.org/wiki/Discovery_Day
	// Journée de la Découverte (CA-YT)
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.August, Month3rd), uint32(time.Monday))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.August, Month3rd), uint32(time.Friday))
	...
	// Fête du Travail
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.September, Month1st), uint32(time.Monday))
	...
	t = find_date(fmt.Sprintf("%d-%02d-30", year, time.September))
	...
	t = find_date(fmt.Sprintf("%d-%02d-10", year, time.October))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.October, Month2nd), uint32(time.Monday))
	...
	t = find_date(fmt.Sprintf("%d-%02d-31", year, time.October))
	print_date(t, "Hallowe'en")
	print_date(t, "All Hallows' Eve")
	t = find_date(fmt.Sprintf("%d-%02d-01", year, time.November))
	print_date(t, "All Hallows' Day")
	print_date(t, "All Saints' Day")
	t = find_date(fmt.Sprintf("%d-%02d-02", year, time.November))
	...
	t = find_date(fmt.Sprintf("%d-%02d-11", year, time.November))
	print_date(t, "Rememberance Day")
	print_date(t, "Armistice Day (CA-NL, GB)")
	print_date(t, "Veterans Day (US)")
	t = find_date(fmt.Sprintf("%d-%02d-03", year, time.September))
	...
	t = find_date(fmt.Sprintf("%d-%02d-11", year, time.December))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-%02d", year, time.December, Month1st), uint32(time.Saturday))
	...
	// La Inmaculada
	t = find_date(fmt.Sprintf("%d-%02d-08", year, time.December))
	...
	t = find_nearby_date(fmt.Sprintf("%d-%02d-21", year, time.December), uint32(time.Sunday))
	t = t.AddDate(0, 0, -21)
	...
	t = find_date(fmt.Sprintf("%d-%02d-24", year, time.December))
	print_date(t, "Christmas Eve")
	t = find_date(fmt.Sprintf("%d-%02d-25", year, time.December))
	...
	t = find_date(fmt.Sprintf("%d-%02d-07", year, time.January))
	print_date(t, "Orthodox Christmas Day")
	t = find_date(fmt.Sprintf("%d-%02d-26", year, time.December))
	...
	// Epiphanytide
	// Épiphanie, Epifanía
	t = find_date(fmt.Sprintf("%d-%02d-05", year, time.January))
	print_date(t, "Epiphany Eve")
	t = find_date(fmt.Sprintf("%d-%02d-06", year, time.January))
	print_date(t, "Epiphany")
	print_date(t, "Reyes")
	t = find_nearby_date(fmt.Sprintf("%d-%02d-10", year, time.January), uint32(time.Sunday))
	...
	// Annonciation
	t = find_date(fmt.Sprintf("%d-%02d-25", year, time.March))
